...
int EveLuaManager::LuaCallMemberFunction(lua_State* L)
{
	...
	for (TFieldIterator<FProperty> It(Function); It; ++It)
	{
		...
		if (Param->IsA(FIntProperty::StaticClass()) && Index < NumParams)
		{
			...
			*Param->ContainerPtrToValuePtr<int32>(Params) = Value;
			UE_LOG(LogTemp, Log, TEXT("[LuaLog] Param %d: %d"), Index, *Param->ContainerPtrToValuePtr<int32>(Params));
			Index++;
		}
	}
	...
	{
		UE_LOG(LogTemp, Log, TEXT("[LuaLog] Begin Params[%d] = %d"), i, ParamsBytes[i]);
	}
	...
	if (ReturnProp)
	{
		UE_LOG(LogTemp, Log, TEXT("[LuaLog] ReturnProp Name: %s, Type: %s"), *ReturnProp->GetName(), *ReturnProp->GetClass()->GetName());
		if (ReturnProp->IsA(FIntProperty::StaticClass()))
		{
			int32 ReturnValue = *ReturnProp->ContainerPtrToValuePtr<int32>(Params);
			UE_LOG(LogTemp, Log, TEXT("[LuaLog] Return Value: %d"), ReturnValue);
	...
	for (int32 i = 0; i < Function->ParmsSize; ++i)
	{
		UE_LOG(LogTemp, Log, TEXT("[LuaLog] End Params[%d] = %d"), i, FinalParamsBytes[i]);
	}
